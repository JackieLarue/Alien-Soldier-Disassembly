...
namespace GunstarCompression
{
    ...
    class MainRunner
    {
        static void Main()
        {
            //string addrs_filepath = @"../tiles/tile_ghd.txt";
            //string rom_filepath = @"../Alien Soldier (J) [!].bin";
            // Opening above file and reading it back.
            var extracter = new Decompressor();
            string[] fileEntries = Directory.GetFiles(@"../../tiles/ghd");
            foreach (string fileName in fileEntries)
            {
                var fi = new FileInfo(fileName);
                using (FileStream fs = File.Open(fileName, FileMode.Open, FileAccess.Read, FileShare.None))
                {
                    byte[] input = new byte[fi.Length];
                    fs.Read(input);
                    extracter.Decompress(input);
                }

                string outFileName = @"../../tiles/ghd/decompressed/" + fi.Name;
                using (FileStream fs = File.Open(outFileName, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    fs.Write(extracter.GetOutput());
                }
