...
class TileLogger:
    ...
    def log_all(self):
        self.log_unique('tile_raw', 0)
        self.log_unique('tile_ghd', 1)
        self.log_unique('tile_rle', 2)

def unpack_rle(data, addr):
    input_data = data[addr:]
    decomp_buffer = array('B')

    input_cursor = 2
    decomp_buffer_cursor = 0
    #size = read_u16(input_data)
    size = input_data[0] * 0x100 + input_data[1]
    print(f'Address: {addr:02X}, size: {size:02X}')

    while input_cursor <= size and input_cursor <= len(input_data):
        control_byte = input_data[input_cursor]
        if control_byte >= 0x80:
            tmp = control_byte
            cnt = ((control_byte >> 2) & 0x1F) + 1
            tmp = (tmp << 8) & 0xFFFF
            input_cursor += 1
            s = input_data[input_cursor]
            tmp += s
            tmp = (tmp & 0x3FF) + 1
            ...
            if (control_byte & (1 << 5)) != 0:
                ...
                if (control_byte & (1 << 6)) != 0:
                    cnt = (control_byte & 0x1F) + 1
                    s1 = input_data[input_cursor + 1]
                    s2 = input_data[input_cursor + 2]
                    input_cursor += 2
                    for _ in range(cnt + 1):
